...
import { CollisionGroups } from "../../pinball/Collision";
import { off } from "process";
import { KeyCode } from "../io/Keys";
import { getBinding } from "../../pinball/ui/KeyboardBindings";
...
export default class DebugRenderer extends BaseEntity implements Entity {
  ...
  onKeyDown(key: KeyCode) {
    if (key === getBinding("TOGGLE_DEBUG_DRAWING")) {
      ...
      for (const body of this.game!.world.bodies) {
        ...
        for (const shape of body.shapes) {
          ...
          if (!shape.collisionResponse || !body.collisionResponse) {
            ctx.fillStyle = "#00fa";
          } else if (shape.collisionGroup === CollisionGroups.Ball) {
            ctx.fillStyle = "#f00";
          } else {
            ctx.fillStyle = "#000";
          }
